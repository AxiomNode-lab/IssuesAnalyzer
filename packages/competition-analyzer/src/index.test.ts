...
describe("analyzeCompetition", () => {
    ...
    const result = analyzeCompetition(
      input({
        comments: [
          ...
          { ...comment("@bob has applied to work on this issue as part of the Wave Program.", "bob"), sourceUrl: `${issueUrl}#b` },
        ],
      }),
    );
    expect(result.status).toBe("possible");
  ...
  it("ignores bot and issue-author claim comments", () => {
    const result = analyzeCompetition(input({ comments: [comment("I am working on this", "automation[bot]"), comment("I am working on this", "maintainer")] }));
    expect(result.status).toBe("none_visible");
    expect(result.score).toBe(0);
  });
  ...
  it("never treats no visible signals as guaranteed availability", () => {
    ...
    expect(result.status).toBe("none_visible");
    expect(result.inferences).toContainEqual(
      expect.objectContaining({ key: "competition.noneActiveVisible", caution: expect.stringContaining("does not guarantee") }),
    );
  });

  it("returns uncertain and lowers confidence when bounded sources are unavailable", () => {
    const result = analyzeCompetition(input({ comments: null, timeline: null, pullRequests: null }));
  ...
  it("does not inflate competition for timeline references that are not pull requests", () => {
    ...
    expect(result.status).toBe("none_visible");
    expect(result.score).toBe(0);
    expect(result.facts).toContainEqual(expect.objectContaining({ key: "competition.nonPullRequestReferenceCount", value: 1 }));
  });

  it("uses GitHub-confirmed timeline PRs and distinguishes active, draft, and historical work", () => {
    const active = pullRequest(null);
    const draft = { ...pullRequest("Closes #42"), number: 8, draft: true, sourceUrl: "https://github.com/example/project/pull/8" };
    const merged = { ...pullRequest(null), number: 9, state: "closed" as const, closedAt: new Date("2026-08-25T00:00:00.000Z"), mergedAt: new Date("2026-08-25T00:00:00.000Z"), sourceUrl: "https://github.com/example/project/pull/9" };
    const result = analyzeCompetition(input({ timeline: [
  ...
  it("treats only old merged or closed PRs as weaker historical competition", () => {
    const historical = { ...pullRequest("Closes #42"), state: "closed" as const, closedAt: new Date("2026-08-25T00:00:00.000Z"), mergedAt: new Date("2026-08-25T00:00:00.000Z") };
  ...
  it("reduces confidence when linked-PR evidence reached a collection bound", () => {
    const result = analyzeCompetition(input({ completeness: { timeline: false, pullRequests: false } }));
    expect(result.score).toBe(0);
    expect(result.confidence).toEqual({ level: "low", value: 40 });
    expect(result.warnings).toContain("Timeline evidence reached its collection bound; additional linked work may exist.");
  });

  it("bounds every evidence window after selecting newest records", () => {
    const comments = Array.from({ length: 510 }, (_, index) => ({ ...comment(null), sourceUrl: `${issueUrl}#issuecomment-${index}`, createdAt: new Date(asOf.getTime() - index * 1_000) }));
    const timeline = Array.from({ length: 310 }, (_, index) => ({ ...event("labeled"), sourceUrl: `${issueUrl}#event-${index}`, createdAt: new Date(asOf.getTime() - index * 1_000) }));
    const pullRequests = Array.from({ length: 110 }, (_, index) => ({ ...pullRequest(null), number: index + 1, sourceUrl: `https://github.com/example/project/pull/${index + 1}`, updatedAt: new Date(asOf.getTime() - index * 1_000) }));
    const result = analyzeCompetition(input({ comments, timeline, pullRequests }));
    expect(result.evidenceWindow).toEqual({ commentsUsed: 500, timelineEventsUsed: 300, pullRequestsUsed: 100 });
    expect(result.warnings).toHaveLength(3);
  });

  it("rejects future evidence and remains deterministic", () => {
    expect(() => analyzeCompetition(input({ comments: [{ ...comment("working on this"), createdAt: new Date("2026-08-27T00:00:00.000Z") }] }))).toThrow(new RangeError("comment date cannot be after asOf."));
