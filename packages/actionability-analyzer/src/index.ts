...
const UNRESOLVED_DIRECTION =
  /\b(?:which (?:option|approach)|what do you think|thoughts\?|open to (?:ideas|suggestions)|happy to hear other approaches|alternatively|possible approaches?|approaches worth considering|one option|another option|we could either|can we move|how about|which one is best|what suggestions)\b/i;
...
const PROPOSAL_STYLE =
  /\b(?:proposal|propose|introduce|motivation|why this is feasible|would like to|could improve|should we|consider adding|consider introducing|can we move|recommend weblate|review this when you are free)\b/i;
const TRIVIAL_CONTRIBUTION =
  /\b(?:under\s+(?:60|sixty)\s+seconds?|less than (?:a|one) minute|takes?\s*<\s*60\s*seconds?|no (?:clone|cloning|install|installation|setup|code|coding)|browser[- ]only|copy(?:\s*\/\s*| and )paste|paste (?:this|the) (?:json|object|snippet)|exact (?:json|object|snippet)|single json object)\b/i;
...
export function analyzeIssueActionability(input: ActionabilityInput): ActionabilityResult {
  ...
  if (unresolvedMaintainerDecision)
    facts.push(fact("actionability.unresolvedMaintainerDecision", true));
  if (migrationScope) facts.push(fact("actionability.migrationRisk", true));
  if (dependencyScope) facts.push(fact("actionability.dependencyRisk", true));
  if (researchScope) facts.push(fact("actionability.researchRisk", true));
  ...
  if (trivialContribution)
    warnings.push(
      "The task appears unusually trivial or copy/paste-oriented; this is easy to execute but has lower contribution value.",
    );
  if (automatedIssue)
    warnings.push(
